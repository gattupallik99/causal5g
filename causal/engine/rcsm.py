...
class RootCauseScoringModule:
    ...
    # Day 17b: persist exited/paused Docker state across score() calls.
    # The pipeline generates its first post-injection report within ~30s;
    # Docker sometimes updates container state 1-2s after that first call.
    # Keeping "exited"/"paused" for _EXITED_PERSIST_CYCLES ensures the
    # correct NF wins even when the first Docker call races with the kill.
    _EXITED_PERSIST_CYCLES = 3

    def __init__(self):
        self.bayesian = BayesianRootCauseLayer()
        self.report_counter = 0
        self._exited_nfs: dict[str, int] = {}  # nf → score_call# when seen
    ...
    def score(
        ...
        # Day 17: query container_status once - used by the per-NF
        ...
        # Day 17b: persist exited/paused state across score cycles.
        # Handles (a) 1-2s Docker state-update race after container kill,
        # (b) pcf_timeout which uses "paused" rather than "exited".
        self._score_calls += 1
        for nf, st in container_status.items():
            if st in ("exited", "paused"):
                self._exited_nfs[nf] = self._score_calls
            elif st == "running" and nf in self._exited_nfs:
                # Container recovered — drop immediately so a restart_pod
                # remediation on scenario N doesn't pollute scenario N+1.
                del self._exited_nfs[nf]
        # Expire entries older than _EXITED_PERSIST_CYCLES calls.
        self._exited_nfs = {
            nf: age for nf, age in self._exited_nfs.items()
            if self._score_calls - age <= self._EXITED_PERSIST_CYCLES
        }
        ...
        for nf in graph.nodes:
            ...
            # Day 17: container-status-based primary boost. Falls back
            # to the Day 13 reachability floor when Docker is
            # unavailable or no container is exited.
            if container_status.get(nf) in ("exited", "paused") or nf in self._exited_nfs:
                # 1.01 beats the Day-13 reachability floor max (0.8 + 0.2*c
                # = 1.0 for NRF) so a crashed non-NRF NF always wins rank 1.
                composite = max(composite, 1.01)
